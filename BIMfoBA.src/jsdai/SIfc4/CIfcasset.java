/* Generated by JSDAI Express Compiler, version 4.3.2, build 500, 2011-12-13 */

// Java class implementing entity IfcAsset

package jsdai.SIfc4;
import jsdai.lang.*;

public class CIfcasset extends CIfcgroup implements EIfcasset {
	public static final jsdai.dictionary.CEntity_definition definition = initEntityDefinition(CIfcasset.class, SIfc4.ss);

	/*----------------------------- Attributes -----------*/

/*
	// GlobalId: protected String a0;   GlobalId - java inheritance - STRING
	// OwnerHistory: protected Object a1;   OwnerHistory - java inheritance - ENTITY IfcOwnerHistory
	// Name: protected String a2;   Name - java inheritance - STRING
	// Description: protected String a3;   Description - java inheritance - STRING
	// HasAssignments: protected Object  - inverse - java inheritance -  ENTITY IfcRelAssigns
	// Nests: protected Object  - inverse - java inheritance -  ENTITY IfcRelNests
	// IsNestedBy: protected Object  - inverse - java inheritance -  ENTITY IfcRelNests
	// HasContext: protected Object  - inverse - java inheritance -  ENTITY IfcRelDeclares
	// IsDecomposedBy: protected Object  - inverse - java inheritance -  ENTITY IfcRelAggregates
	// Decomposes: protected Object  - inverse - java inheritance -  ENTITY IfcRelAggregates
	// HasAssociations: protected Object  - inverse - java inheritance -  ENTITY IfcRelAssociates
	// ObjectType: protected String a4;   ObjectType - java inheritance - STRING
	// IsDeclaredBy: protected Object  - inverse - java inheritance -  ENTITY IfcRelDefinesByObject
	// Declares: protected Object  - inverse - java inheritance -  ENTITY IfcRelDefinesByObject
	// IsTypedBy: protected Object  - inverse - java inheritance -  ENTITY IfcRelDefinesByType
	// IsDefinedBy: protected Object  - inverse - java inheritance -  ENTITY IfcRelDefinesByProperties
	// IsGroupedBy: protected Object  - inverse - java inheritance -  ENTITY IfcRelAssignsToGroup
	protected String a5; // Identification - current entity - STRING
	protected static final jsdai.dictionary.CExplicit_attribute a5$ = CEntity.initExplicitAttribute(definition, 5);
	protected Object a6; // OriginalValue - current entity - ENTITY IfcCostValue
	protected static final jsdai.dictionary.CExplicit_attribute a6$ = CEntity.initExplicitAttribute(definition, 6);
	protected Object a7; // CurrentValue - current entity - ENTITY IfcCostValue
	protected static final jsdai.dictionary.CExplicit_attribute a7$ = CEntity.initExplicitAttribute(definition, 7);
	protected Object a8; // TotalReplacementCost - current entity - ENTITY IfcCostValue
	protected static final jsdai.dictionary.CExplicit_attribute a8$ = CEntity.initExplicitAttribute(definition, 8);
	protected Object a9; // Owner - current entity - SELECT IfcActorSelect
	protected static final jsdai.dictionary.CExplicit_attribute a9$ = CEntity.initExplicitAttribute(definition, 9);
	protected Object a10; // User - current entity - SELECT IfcActorSelect
	protected static final jsdai.dictionary.CExplicit_attribute a10$ = CEntity.initExplicitAttribute(definition, 10);
	protected Object a11; // ResponsiblePerson - current entity - ENTITY IfcPerson
	protected static final jsdai.dictionary.CExplicit_attribute a11$ = CEntity.initExplicitAttribute(definition, 11);
	protected String a12; // IncorporationDate - current entity - STRING
	protected static final jsdai.dictionary.CExplicit_attribute a12$ = CEntity.initExplicitAttribute(definition, 12);
	protected Object a13; // DepreciatedValue - current entity - ENTITY IfcCostValue
	protected static final jsdai.dictionary.CExplicit_attribute a13$ = CEntity.initExplicitAttribute(definition, 13);
*/

	/*----------------------------- Attributes (new version) -----------*/

	// GlobalId - explicit - java inheritance
	// protected static final jsdai.dictionary.CExplicit_attribute a0$ = CEntity.initExplicitAttribute(definition, 0);
	// protected String a0;
	// OwnerHistory - explicit - java inheritance
	// protected static final jsdai.dictionary.CExplicit_attribute a1$ = CEntity.initExplicitAttribute(definition, 1);
	// protected Object a1;
	// Name - explicit - java inheritance
	// protected static final jsdai.dictionary.CExplicit_attribute a2$ = CEntity.initExplicitAttribute(definition, 2);
	// protected String a2;
	// Description - explicit - java inheritance
	// protected static final jsdai.dictionary.CExplicit_attribute a3$ = CEntity.initExplicitAttribute(definition, 3);
	// protected String a3;
	// HasAssignments - inverse - java inheritance
	// protected static final jsdai.dictionary.CInverse_attribute i0$ = CEntity.initInverseAttribute(definition, 0);
	// Nests - inverse - java inheritance
	// protected static final jsdai.dictionary.CInverse_attribute i1$ = CEntity.initInverseAttribute(definition, 1);
	// IsNestedBy - inverse - java inheritance
	// protected static final jsdai.dictionary.CInverse_attribute i2$ = CEntity.initInverseAttribute(definition, 2);
	// HasContext - inverse - java inheritance
	// protected static final jsdai.dictionary.CInverse_attribute i3$ = CEntity.initInverseAttribute(definition, 3);
	// IsDecomposedBy - inverse - java inheritance
	// protected static final jsdai.dictionary.CInverse_attribute i4$ = CEntity.initInverseAttribute(definition, 4);
	// Decomposes - inverse - java inheritance
	// protected static final jsdai.dictionary.CInverse_attribute i5$ = CEntity.initInverseAttribute(definition, 5);
	// HasAssociations - inverse - java inheritance
	// protected static final jsdai.dictionary.CInverse_attribute i6$ = CEntity.initInverseAttribute(definition, 6);
	// ObjectType - explicit - java inheritance
	// protected static final jsdai.dictionary.CExplicit_attribute a4$ = CEntity.initExplicitAttribute(definition, 4);
	// protected String a4;
	// IsDeclaredBy - inverse - java inheritance
	// protected static final jsdai.dictionary.CInverse_attribute i7$ = CEntity.initInverseAttribute(definition, 7);
	// Declares - inverse - java inheritance
	// protected static final jsdai.dictionary.CInverse_attribute i8$ = CEntity.initInverseAttribute(definition, 8);
	// IsTypedBy - inverse - java inheritance
	// protected static final jsdai.dictionary.CInverse_attribute i9$ = CEntity.initInverseAttribute(definition, 9);
	// IsDefinedBy - inverse - java inheritance
	// protected static final jsdai.dictionary.CInverse_attribute i10$ = CEntity.initInverseAttribute(definition, 10);
	// IsGroupedBy - inverse - java inheritance
	// protected static final jsdai.dictionary.CInverse_attribute i11$ = CEntity.initInverseAttribute(definition, 11);
	// Identification - explicit - current entity
	protected static final jsdai.dictionary.CExplicit_attribute a5$ = CEntity.initExplicitAttribute(definition, 5);
	protected String a5;
	// OriginalValue - explicit - current entity
	protected static final jsdai.dictionary.CExplicit_attribute a6$ = CEntity.initExplicitAttribute(definition, 6);
	protected Object a6;
	// CurrentValue - explicit - current entity
	protected static final jsdai.dictionary.CExplicit_attribute a7$ = CEntity.initExplicitAttribute(definition, 7);
	protected Object a7;
	// TotalReplacementCost - explicit - current entity
	protected static final jsdai.dictionary.CExplicit_attribute a8$ = CEntity.initExplicitAttribute(definition, 8);
	protected Object a8;
	// Owner - explicit - current entity
	protected static final jsdai.dictionary.CExplicit_attribute a9$ = CEntity.initExplicitAttribute(definition, 9);
	protected Object a9;
	// User - explicit - current entity
	protected static final jsdai.dictionary.CExplicit_attribute a10$ = CEntity.initExplicitAttribute(definition, 10);
	protected Object a10;
	// ResponsiblePerson - explicit - current entity
	protected static final jsdai.dictionary.CExplicit_attribute a11$ = CEntity.initExplicitAttribute(definition, 11);
	protected Object a11;
	// IncorporationDate - explicit - current entity
	protected static final jsdai.dictionary.CExplicit_attribute a12$ = CEntity.initExplicitAttribute(definition, 12);
	protected String a12;
	// DepreciatedValue - explicit - current entity
	protected static final jsdai.dictionary.CExplicit_attribute a13$ = CEntity.initExplicitAttribute(definition, 13);
	protected Object a13;

	public jsdai.dictionary.EEntity_definition getInstanceType() {
		return definition;
	}

/* *** old implementation ***

	protected void changeReferences(InverseEntity old, InverseEntity newer) throws SdaiException {
		super.changeReferences(old, newer);
		if (a6 == old) {
			a6 = newer;
		}
		if (a7 == old) {
			a7 = newer;
		}
		if (a8 == old) {
			a8 = newer;
		}
		if (a9 == old) {
			a9 = newer;
		}
		if (a10 == old) {
			a10 = newer;
		}
		if (a11 == old) {
			a11 = newer;
		}
		if (a13 == old) {
			a13 = newer;
		}
	}
*/


	protected void changeReferences(InverseEntity old, InverseEntity newer) throws SdaiException {
		super.changeReferences(old, newer);
		if (a6 == old) {
			a6 = newer;
		}
		if (a7 == old) {
			a7 = newer;
		}
		if (a8 == old) {
			a8 = newer;
		}
		if (a9 == old) {
			a9 = newer;
		}
		if (a10 == old) {
			a10 = newer;
		}
		if (a11 == old) {
			a11 = newer;
		}
		if (a13 == old) {
			a13 = newer;
		}
	}

	/*----------- Methods for attribute access -----------*/


	/*----------- Methods for attribute access (new)-----------*/

	//going through all the attributes: #5618=EXPLICIT_ATTRIBUTE('GlobalId',#5616,0,#2517,$,.F.);
	//<01> generating methods for consolidated attribute:  GlobalId
	//<01-1> supertype, java inheritance
	//<01-1-0> explicit - generateExplicitSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #5619=EXPLICIT_ATTRIBUTE('OwnerHistory',#5616,1,#4858,$,.T.);
	//<01> generating methods for consolidated attribute:  OwnerHistory
	//<01-1> supertype, java inheritance
	//<01-1-0> explicit - generateExplicitSupertypeJavaInheritedMethodsX()
	// attribute (java explicit): OwnerHistory, base type: entity IfcOwnerHistory
	public static int usedinOwnerhistory(EIfcroot type, EIfcownerhistory instance, ASdaiModel domain, AEntity result) throws SdaiException {
		return ((CEntity)instance).makeUsedin(definition, a1$, domain, result);
	}
	//going through all the attributes: #5620=EXPLICIT_ATTRIBUTE('Name',#5616,2,#2539,$,.T.);
	//<01> generating methods for consolidated attribute:  Name
	//<01-1> supertype, java inheritance
	//<01-1-0> explicit - generateExplicitSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #5621=EXPLICIT_ATTRIBUTE('Description',#5616,3,#2657,$,.T.);
	//<01> generating methods for consolidated attribute:  Description
	//<01-1> supertype, java inheritance
	//<01-1-0> explicit - generateExplicitSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4791=INVERSE_ATTRIBUTE('HasAssignments',#4789,0,#5348,$,#5350,#8672,$,.F.);
	//<01> generating methods for consolidated attribute:  HasAssignments
	//<01-1> supertype, java inheritance
	//<01-1-2> inverse - generateInverseSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4792=INVERSE_ATTRIBUTE('Nests',#4789,1,#5489,$,#5492,#8674,#8675,.F.);
	//<01> generating methods for consolidated attribute:  Nests
	//<01-1> supertype, java inheritance
	//<01-1-2> inverse - generateInverseSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4793=INVERSE_ATTRIBUTE('IsNestedBy',#4789,2,#5489,$,#5491,#8677,$,.F.);
	//<01> generating methods for consolidated attribute:  IsNestedBy
	//<01-1> supertype, java inheritance
	//<01-1-2> inverse - generateInverseSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4794=INVERSE_ATTRIBUTE('HasContext',#4789,3,#5450,$,#5453,#8679,#8680,.F.);
	//<01> generating methods for consolidated attribute:  HasContext
	//<01-1> supertype, java inheritance
	//<01-1-2> inverse - generateInverseSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4795=INVERSE_ATTRIBUTE('IsDecomposedBy',#4789,4,#5344,$,#5346,#8682,$,.F.);
	//<01> generating methods for consolidated attribute:  IsDecomposedBy
	//<01-1> supertype, java inheritance
	//<01-1-2> inverse - generateInverseSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4796=INVERSE_ATTRIBUTE('Decomposes',#4789,5,#5344,$,#5347,#8684,#8685,.F.);
	//<01> generating methods for consolidated attribute:  Decomposes
	//<01-1> supertype, java inheritance
	//<01-1-2> inverse - generateInverseSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4797=INVERSE_ATTRIBUTE('HasAssociations',#4789,6,#5375,$,#5377,#8687,$,.F.);
	//<01> generating methods for consolidated attribute:  HasAssociations
	//<01-1> supertype, java inheritance
	//<01-1-2> inverse - generateInverseSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4784=EXPLICIT_ATTRIBUTE('ObjectType',#4782,0,#2539,$,.T.);
	//<01> generating methods for consolidated attribute:  ObjectType
	//<01-1> supertype, java inheritance
	//<01-1-0> explicit - generateExplicitSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4785=INVERSE_ATTRIBUTE('IsDeclaredBy',#4782,0,#5458,$,#5460,#8660,#8661,.F.);
	//<01> generating methods for consolidated attribute:  IsDeclaredBy
	//<01-1> supertype, java inheritance
	//<01-1-2> inverse - generateInverseSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4786=INVERSE_ATTRIBUTE('Declares',#4782,1,#5458,$,#5461,#8663,$,.F.);
	//<01> generating methods for consolidated attribute:  Declares
	//<01-1> supertype, java inheritance
	//<01-1-2> inverse - generateInverseSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4787=INVERSE_ATTRIBUTE('IsTypedBy',#4782,2,#5470,$,#5472,#8665,#8666,.F.);
	//<01> generating methods for consolidated attribute:  IsTypedBy
	//<01-1> supertype, java inheritance
	//<01-1-2> inverse - generateInverseSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4788=INVERSE_ATTRIBUTE('IsDefinedBy',#4782,3,#5462,$,#5464,#8668,$,.F.);
	//<01> generating methods for consolidated attribute:  IsDefinedBy
	//<01-1> supertype, java inheritance
	//<01-1-2> inverse - generateInverseSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #4449=INVERSE_ATTRIBUTE('IsGroupedBy',#4447,0,#5359,$,#5361,#8503,$,.F.);
	//<01> generating methods for consolidated attribute:  IsGroupedBy
	//<01-1> supertype, java inheritance
	//<01-1-2> inverse - generateInverseSupertypeJavaInheritedMethodsX()
	//going through all the attributes: #3345=EXPLICIT_ATTRIBUTE('Identification',#3343,0,#2523,$,.T.);
	//<01> generating methods for consolidated attribute:  Identification
	//<01-0> current entity
	//<01-0-0> explicit attribute - generateExplicitCurrentEntityMethodsX()
	/// methods for attribute: Identification, base type: STRING
	public boolean testIdentification(EIfcasset type) throws SdaiException {
		return test_string(a5);
	}
	public String getIdentification(EIfcasset type) throws SdaiException {
		return get_string(a5);
	}
	public void setIdentification(EIfcasset type, String value) throws SdaiException {
		a5 = set_string(value);
	}
	public void unsetIdentification(EIfcasset type) throws SdaiException {
		a5 = unset_string();
	}
	public static jsdai.dictionary.EAttribute attributeIdentification(EIfcasset type) throws SdaiException {
		return a5$;
	}

	//going through all the attributes: #3346=EXPLICIT_ATTRIBUTE('OriginalValue',#3343,1,#3847,$,.T.);
	//<01> generating methods for consolidated attribute:  OriginalValue
	//<01-0> current entity
	//<01-0-0> explicit attribute - generateExplicitCurrentEntityMethodsX()
	// attribute (current explicit or supertype explicit) : OriginalValue, base type: entity IfcCostValue
	public static int usedinOriginalvalue(EIfcasset type, EIfccostvalue instance, ASdaiModel domain, AEntity result) throws SdaiException {
		return ((CEntity)instance).makeUsedin(definition, a6$, domain, result);
	}
	public boolean testOriginalvalue(EIfcasset type) throws SdaiException {
		return test_instance(a6);
	}
	public EIfccostvalue getOriginalvalue(EIfcasset type) throws SdaiException {
		return (EIfccostvalue)get_instance(a6);
	}
	public void setOriginalvalue(EIfcasset type, EIfccostvalue value) throws SdaiException {
		a6 = set_instance(a6, value);
	}
	public void unsetOriginalvalue(EIfcasset type) throws SdaiException {
		a6 = unset_instance(a6);
	}
	public static jsdai.dictionary.EAttribute attributeOriginalvalue(EIfcasset type) throws SdaiException {
		return a6$;
	}

	//going through all the attributes: #3347=EXPLICIT_ATTRIBUTE('CurrentValue',#3343,2,#3847,$,.T.);
	//<01> generating methods for consolidated attribute:  CurrentValue
	//<01-0> current entity
	//<01-0-0> explicit attribute - generateExplicitCurrentEntityMethodsX()
	// attribute (current explicit or supertype explicit) : CurrentValue, base type: entity IfcCostValue
	public static int usedinCurrentvalue(EIfcasset type, EIfccostvalue instance, ASdaiModel domain, AEntity result) throws SdaiException {
		return ((CEntity)instance).makeUsedin(definition, a7$, domain, result);
	}
	public boolean testCurrentvalue(EIfcasset type) throws SdaiException {
		return test_instance(a7);
	}
	public EIfccostvalue getCurrentvalue(EIfcasset type) throws SdaiException {
		return (EIfccostvalue)get_instance(a7);
	}
	public void setCurrentvalue(EIfcasset type, EIfccostvalue value) throws SdaiException {
		a7 = set_instance(a7, value);
	}
	public void unsetCurrentvalue(EIfcasset type) throws SdaiException {
		a7 = unset_instance(a7);
	}
	public static jsdai.dictionary.EAttribute attributeCurrentvalue(EIfcasset type) throws SdaiException {
		return a7$;
	}

	//going through all the attributes: #3348=EXPLICIT_ATTRIBUTE('TotalReplacementCost',#3343,3,#3847,$,.T.);
	//<01> generating methods for consolidated attribute:  TotalReplacementCost
	//<01-0> current entity
	//<01-0-0> explicit attribute - generateExplicitCurrentEntityMethodsX()
	// attribute (current explicit or supertype explicit) : TotalReplacementCost, base type: entity IfcCostValue
	public static int usedinTotalreplacementcost(EIfcasset type, EIfccostvalue instance, ASdaiModel domain, AEntity result) throws SdaiException {
		return ((CEntity)instance).makeUsedin(definition, a8$, domain, result);
	}
	public boolean testTotalreplacementcost(EIfcasset type) throws SdaiException {
		return test_instance(a8);
	}
	public EIfccostvalue getTotalreplacementcost(EIfcasset type) throws SdaiException {
		return (EIfccostvalue)get_instance(a8);
	}
	public void setTotalreplacementcost(EIfcasset type, EIfccostvalue value) throws SdaiException {
		a8 = set_instance(a8, value);
	}
	public void unsetTotalreplacementcost(EIfcasset type) throws SdaiException {
		a8 = unset_instance(a8);
	}
	public static jsdai.dictionary.EAttribute attributeTotalreplacementcost(EIfcasset type) throws SdaiException {
		return a8$;
	}

	//going through all the attributes: #3349=EXPLICIT_ATTRIBUTE('Owner',#3343,4,#3111,$,.T.);
	//<01> generating methods for consolidated attribute:  Owner
	//<01-0> current entity
	//<01-0-0> explicit attribute - generateExplicitCurrentEntityMethodsX()
	// -2- methods for SELECT attribute: Owner
	public static int usedinOwner(EIfcasset type, EEntity instance, ASdaiModel domain, AEntity result) throws SdaiException {
		return ((CEntity)instance).makeUsedin(definition, a9$, domain, result);
	}
	public boolean testOwner(EIfcasset type) throws SdaiException {
		return test_instance(a9);
	}

	public EEntity getOwner(EIfcasset type) throws SdaiException { // case 1
		return get_instance_select(a9);
	}

	public void setOwner(EIfcasset type, EEntity value) throws SdaiException { // case 1
		a9 = set_instance(a9, value);
	}

	public void unsetOwner(EIfcasset type) throws SdaiException {
		a9 = unset_instance(a9);
	}

	public static jsdai.dictionary.EAttribute attributeOwner(EIfcasset type) throws SdaiException {
		return a9$;
	}

	//going through all the attributes: #3350=EXPLICIT_ATTRIBUTE('User',#3343,5,#3111,$,.T.);
	//<01> generating methods for consolidated attribute:  User
	//<01-0> current entity
	//<01-0-0> explicit attribute - generateExplicitCurrentEntityMethodsX()
	// -2- methods for SELECT attribute: User
	public static int usedinUser(EIfcasset type, EEntity instance, ASdaiModel domain, AEntity result) throws SdaiException {
		return ((CEntity)instance).makeUsedin(definition, a10$, domain, result);
	}
	public boolean testUser(EIfcasset type) throws SdaiException {
		return test_instance(a10);
	}

	public EEntity getUser(EIfcasset type) throws SdaiException { // case 1
		return get_instance_select(a10);
	}

	public void setUser(EIfcasset type, EEntity value) throws SdaiException { // case 1
		a10 = set_instance(a10, value);
	}

	public void unsetUser(EIfcasset type) throws SdaiException {
		a10 = unset_instance(a10);
	}

	public static jsdai.dictionary.EAttribute attributeUser(EIfcasset type) throws SdaiException {
		return a10$;
	}

	//going through all the attributes: #3351=EXPLICIT_ATTRIBUTE('ResponsiblePerson',#3343,6,#4894,$,.T.);
	//<01> generating methods for consolidated attribute:  ResponsiblePerson
	//<01-0> current entity
	//<01-0-0> explicit attribute - generateExplicitCurrentEntityMethodsX()
	// attribute (current explicit or supertype explicit) : ResponsiblePerson, base type: entity IfcPerson
	public static int usedinResponsibleperson(EIfcasset type, EIfcperson instance, ASdaiModel domain, AEntity result) throws SdaiException {
		return ((CEntity)instance).makeUsedin(definition, a11$, domain, result);
	}
	public boolean testResponsibleperson(EIfcasset type) throws SdaiException {
		return test_instance(a11);
	}
	public EIfcperson getResponsibleperson(EIfcasset type) throws SdaiException {
		return (EIfcperson)get_instance(a11);
	}
	public void setResponsibleperson(EIfcasset type, EIfcperson value) throws SdaiException {
		a11 = set_instance(a11, value);
	}
	public void unsetResponsibleperson(EIfcasset type) throws SdaiException {
		a11 = unset_instance(a11);
	}
	public static jsdai.dictionary.EAttribute attributeResponsibleperson(EIfcasset type) throws SdaiException {
		return a11$;
	}

	//going through all the attributes: #3352=EXPLICIT_ATTRIBUTE('IncorporationDate',#3343,7,#2475,$,.T.);
	//<01> generating methods for consolidated attribute:  IncorporationDate
	//<01-0> current entity
	//<01-0-0> explicit attribute - generateExplicitCurrentEntityMethodsX()
	/// methods for attribute: IncorporationDate, base type: STRING
	public boolean testIncorporationdate(EIfcasset type) throws SdaiException {
		return test_string(a12);
	}
	public String getIncorporationdate(EIfcasset type) throws SdaiException {
		return get_string(a12);
	}
	public void setIncorporationdate(EIfcasset type, String value) throws SdaiException {
		a12 = set_string(value);
	}
	public void unsetIncorporationdate(EIfcasset type) throws SdaiException {
		a12 = unset_string();
	}
	public static jsdai.dictionary.EAttribute attributeIncorporationdate(EIfcasset type) throws SdaiException {
		return a12$;
	}

	//going through all the attributes: #3353=EXPLICIT_ATTRIBUTE('DepreciatedValue',#3343,8,#3847,$,.T.);
	//<01> generating methods for consolidated attribute:  DepreciatedValue
	//<01-0> current entity
	//<01-0-0> explicit attribute - generateExplicitCurrentEntityMethodsX()
	// attribute (current explicit or supertype explicit) : DepreciatedValue, base type: entity IfcCostValue
	public static int usedinDepreciatedvalue(EIfcasset type, EIfccostvalue instance, ASdaiModel domain, AEntity result) throws SdaiException {
		return ((CEntity)instance).makeUsedin(definition, a13$, domain, result);
	}
	public boolean testDepreciatedvalue(EIfcasset type) throws SdaiException {
		return test_instance(a13);
	}
	public EIfccostvalue getDepreciatedvalue(EIfcasset type) throws SdaiException {
		return (EIfccostvalue)get_instance(a13);
	}
	public void setDepreciatedvalue(EIfcasset type, EIfccostvalue value) throws SdaiException {
		a13 = set_instance(a13, value);
	}
	public void unsetDepreciatedvalue(EIfcasset type) throws SdaiException {
		a13 = unset_instance(a13);
	}
	public static jsdai.dictionary.EAttribute attributeDepreciatedvalue(EIfcasset type) throws SdaiException {
		return a13$;
	}


	/*---------------------- setAll() --------------------*/

/* *** old implementation ***
	protected void setAll(ComplexEntityValue av) throws SdaiException {
		if (av == null) {
			a5 = null;
			a6 = unset_instance(a6);
			a7 = unset_instance(a7);
			a8 = unset_instance(a8);
			a9 = unset_instance(a9);
			a10 = unset_instance(a10);
			a11 = unset_instance(a11);
			a12 = null;
			a13 = unset_instance(a13);
			a4 = null;
			a0 = null;
			a1 = unset_instance(a1);
			a2 = null;
			a3 = null;
			return;
		}
		a5 = av.entityValues[0].getString(0);
		a6 = av.entityValues[0].getInstance(1, this, a6$);
		a7 = av.entityValues[0].getInstance(2, this, a7$);
		a8 = av.entityValues[0].getInstance(3, this, a8$);
		a9 = av.entityValues[0].getInstance(4, this, a9$);
		a10 = av.entityValues[0].getInstance(5, this, a10$);
		a11 = av.entityValues[0].getInstance(6, this, a11$);
		a12 = av.entityValues[0].getString(7);
		a13 = av.entityValues[0].getInstance(8, this, a13$);
		a4 = av.entityValues[2].getString(0);
		a0 = av.entityValues[4].getString(0);
		a1 = av.entityValues[4].getInstance(1, this, a1$);
		a2 = av.entityValues[4].getString(2);
		a3 = av.entityValues[4].getString(3);
	}
*/

	protected void setAll(ComplexEntityValue av) throws SdaiException {
		if (av == null) {
			a5 = null;
			a6 = unset_instance(a6);
			a7 = unset_instance(a7);
			a8 = unset_instance(a8);
			a9 = unset_instance(a9);
			a10 = unset_instance(a10);
			a11 = unset_instance(a11);
			a12 = null;
			a13 = unset_instance(a13);
			a4 = null;
			a0 = null;
			a1 = unset_instance(a1);
			a2 = null;
			a3 = null;
			return;
		}
		a5 = av.entityValues[0].getString(0);
		a6 = av.entityValues[0].getInstance(1, this, a6$);
		a7 = av.entityValues[0].getInstance(2, this, a7$);
		a8 = av.entityValues[0].getInstance(3, this, a8$);
		a9 = av.entityValues[0].getInstance(4, this, a9$);
		a10 = av.entityValues[0].getInstance(5, this, a10$);
		a11 = av.entityValues[0].getInstance(6, this, a11$);
		a12 = av.entityValues[0].getString(7);
		a13 = av.entityValues[0].getInstance(8, this, a13$);
		a4 = av.entityValues[2].getString(0);
		a0 = av.entityValues[4].getString(0);
		a1 = av.entityValues[4].getInstance(1, this, a1$);
		a2 = av.entityValues[4].getString(2);
		a3 = av.entityValues[4].getString(3);
	}

	/*---------------------- getAll() --------------------*/

/* *** old implementation ***
	protected void getAll(ComplexEntityValue av) throws SdaiException {
		// partial entity: IfcAsset
		av.entityValues[0].setString(0, a5);
		av.entityValues[0].setInstance(1, a6);
		av.entityValues[0].setInstance(2, a7);
		av.entityValues[0].setInstance(3, a8);
		av.entityValues[0].setInstance(4, a9);
		av.entityValues[0].setInstance(5, a10);
		av.entityValues[0].setInstance(6, a11);
		av.entityValues[0].setString(7, a12);
		av.entityValues[0].setInstance(8, a13);
		// partial entity: IfcGroup
		// partial entity: IfcObject
		av.entityValues[2].setString(0, a4);
		// partial entity: IfcObjectDefinition
		// partial entity: IfcRoot
		av.entityValues[4].setString(0, a0);
		av.entityValues[4].setInstance(1, a1);
		av.entityValues[4].setString(2, a2);
		av.entityValues[4].setString(3, a3);
	}
*/

	protected void getAll(ComplexEntityValue av) throws SdaiException {
		// partial entity: IfcAsset
		av.entityValues[0].setString(0, a5);
		av.entityValues[0].setInstance(1, a6);
		av.entityValues[0].setInstance(2, a7);
		av.entityValues[0].setInstance(3, a8);
		av.entityValues[0].setInstance(4, a9);
		av.entityValues[0].setInstance(5, a10);
		av.entityValues[0].setInstance(6, a11);
		av.entityValues[0].setString(7, a12);
		av.entityValues[0].setInstance(8, a13);
		// partial entity: IfcGroup
		// partial entity: IfcObject
		av.entityValues[2].setString(0, a4);
		// partial entity: IfcObjectDefinition
		// partial entity: IfcRoot
		av.entityValues[4].setString(0, a0);
		av.entityValues[4].setInstance(1, a1);
		av.entityValues[4].setString(2, a2);
		av.entityValues[4].setString(3, a3);
	}
}
